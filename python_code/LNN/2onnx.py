...
if __name__ == '__main__':
    ...
    # -------------------- 导出 ONNX 模型 --------------------
    original_device = next(model.parameters()).device  # 保存当前设备（如 cuda:0）
    model.to('cpu')
    model.eval()

    dummy_input = torch.randn(1, 12, device='cpu')
    onnx_filename = "imu_fusion_mlp.onnx"
    torch.onnx.export(
    ...
    with torch.no_grad():
        for x, y in test_loader:
            x = x.to(original_device)  # 确保数据与模型在同一设备
