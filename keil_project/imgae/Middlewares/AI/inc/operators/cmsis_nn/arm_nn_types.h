...
typedef enum
{
    ...
    ARM_CMSIS_NN_ARG_ERROR = 16,     /**< One or more arguments are incorrect */
    ARM_CMSIS_NN_NO_IMPL_ERROR = 17, /**<  No implementation available */
    ARM_CMSIS_NN_FAILURE = 18,       /**<  Logical error */
} arm_cmsis_nn_status;
...
/** Parameters for integer LSTM, as defined in TFLM from <6.0.0 version */
//typedef struct
//{
...
/** CMSIS-NN object for LSTM parameters  from verison 7.0.0*/
...
typedef struct
{
    ...
    int32_t hidden_size; /**< Size of output from the LSTM cell, used as output and recursively into the next time step*/
    ...
    int32_t execute_status; /* for cur layer lstm's execute_status: initial or has already execute */
...
typedef struct
{
    const uint16_t adj_x;
    const uint16_t adj_y;
